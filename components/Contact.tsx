...
const Contact: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const text = encodeURIComponent(
      `Halo Mas Fahmi / LocaGo Creative! Saya ingin konsultasi proyek.\n\n*Nama:* ${formData.name}\n*WhatsApp:* ${formData.phone}\n*Email:* ${formData.email}\n*Pekerjaan/Bisnis:* ${formData.job}\n*Minat Proyek:* ${formData.package}\n\n*Detail Pesan:*\n${formData.message}`
    );

    // Redirect to WhatsApp Mas Fahmi
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 md:p-20 shadow-2xl flex flex-col md:flex-row items-center gap-12">
          <div className="md:w-1/2">
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-4">
                <div className="w-12 h-12 bg-green-50 dark:bg-green-900/30 text-green-600 rounded-full flex items-center justify-center">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-600 uppercase tracking-widest">{t.contact.whatsapp}</p>
                  <a href="https://wa.me/62895336377648" target="_blank" rel="noopener noreferrer" className="text-lg font-bold text-slate-800 dark:text-white hover:text-green-500 transition-colors">+62 895-3363-77648</a>
                </div>
              </div>
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-600 uppercase tracking-widest">{t.contact.email}</p>
                  <a href="mailto:locagocreative@gmail.com" className="text-lg font-bold text-slate-800 dark:text-white hover:text-blue-500 transition-colors">locagocreative@gmail.com</a>
                </div>
              </div>
              <div className="flex items-center space-x-4">
                <div className="w-12 h-12 bg-cyan-50 dark:bg-cyan-900/30 text-cyan-600 rounded-full flex items-center justify-center">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest">{language === 'en' ? 'Location' : 'Lokasi'}</p>
                  <p className="text-lg font-bold text-slate-800 dark:text-white">{language === 'en' ? 'Garut, West Java, Indonesia' : 'Garut, Jawa Barat, Indonesia'}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <form className="md:w-1/2 w-full space-y-4" onSubmit={handleSubmit}>
            ...
            >
              {isSubmitting ? (language === 'en' ? 'Sending...' : 'Mengirim...') : t.contact.form.submit}
            </button>
          </form>
        </div>
